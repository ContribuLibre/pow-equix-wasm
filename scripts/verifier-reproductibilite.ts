...
// reconstruit depuis une copie des sources placée ailleurs, avec son propre
...
import { construireModule, sha256 } from './construire-wasm.ts'
...
try {
  ...
  const reconstruit = sha256(construireModule(copie, join(copie, 'cible')))
  ...
  console.log(`✓ construction reproductible (sha256 ${reference}${attendu ? ', conforme à l’empreinte attendue' : ''})`)
} finally {
